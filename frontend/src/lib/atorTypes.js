...
export const ATOR_TYPES = ATOR_AREAS.flatMap((area) =>
    area.types.map((type) => ({
        ...type,
        area: area.id,
...
export function colorForType(typeId) {
    return ATOR_TYPES.find((type) => type.id === typeId)?.color ?? '#f48634'
}

export function labelForType(typeId) {
    return ATOR_TYPES.find((type) => type.id === typeId)?.label ?? 'Ator'
}

// Opções e rótulos das ÁREAS para os filtros do catálogo de atores. Derivados de
// ATOR_AREAS (fonte única) — evita redeclarar as 4 áreas dentro da página.
export const ATOR_AREA_OPTIONS = [
    { value: 'todos', label: 'Todos' },
    ...ATOR_AREAS.map((area) => ({ value: area.id, label: area.label })),
]

export const ATOR_AREA_LABELS = Object.fromEntries(ATOR_AREAS.map((area) => [area.id, area.label]))
